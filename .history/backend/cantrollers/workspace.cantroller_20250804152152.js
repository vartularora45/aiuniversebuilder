...
// Get all workspaces of user with pagination and filters
export const getWorkspaces = async (req, res) => {
  try {
    ...
    // Add additional stats for each workspace
...
// Helper function to get workspace statistics
const getWorkspaceStats = async (workspaceId) => {
  const workspace = await Workspace.findById(workspaceId);
  return {
    memberCount: workspace.members.length,
    activeMembers: workspace.members.filter(m => 
      new Date(m.lastActive).getTime() > Date.now() - (30 * 24 * 60 * 60 * 1000)
    ).length,
    totalProjects: workspace.stats.totalProjects,
    activeProjects: workspace.stats.activeProjects,
    lastActivityDate: workspace.lastActive
  };
};

// Add member to workspace with enhanced validation and notifications
...
export const getWorkspaceActivity = async (req, res) => {
  try {
    ...
    // Get workspace statistics and activity
